...
**问题：ddp_overlap_bucketed、ddp_bucketed_benchemarking**
...
| overlap_individual      | -         | 57.770       | 1.268             | 2.19              |
| bucketed_overlap_1mb    | 1         | 57.282       | 1.127             | 1.97              |
| bucketed_overlap_10mb   | 10        | 57.466       | 1.166             | 2.03              |
| bucketed_overlap_100mb  | 100       | 56.277       | 1.814             | 3.22              |
| bucketed_overlap_1000mb | 1000      | 66.087       | 2.199             | 3.34              |

**结果解读**

- `individual` 和 `flat` 的尾部同步时间很高，因为它们都在 backward 后集中通信。
- `overlap_individual` 和 `bucketed_overlap` 的 `sync_tail` 都很低，说明通信大部分被 backward 计算覆盖。
- 在这个 small 配置下，`bucketed_overlap_100mb` 的 step time 最低，但差异很小；small 模型不能完全代表 XL，因为 bucket 数量、参数规模和通信带宽利用率不同。
- `1000MB` bucket 退化得更接近“一个大 bucket”，overlap 机会减少，所以 step time 回升。

**评论**

- Bucketed overlap 同时减少通信调用次数并允许通信与 backward 重叠；在本机 small 配置中，bucketed overlap 将 tail sync 从 naive individual 的 `35.2 ms` 降到约 `1-2 ms`。
- bucket 过大时更接近 flat all-reduce，需要等待更多梯度 ready 才能启动通信，因此 overlap 机会减少；bucket 过小时通信调用次数增加，因此存在折中。

**模型化公式** 令总梯度大小为 `s`，带宽为 `w`，每次通信固定开销为 `o`，bucket 数为 `n_b`。一个简单的通信尾部开销模型是：

$$
H(n_b) = \frac{s}{(n_b \times w)} + n_b \times o
$$

第一项表示最后一个 bucket 的数据传输尾部，第二项表示通信调用开销。令导数为 0：

$$
n_b^\star = \sqrt{\frac{s} {w \times o}}
$$

因此最优 bucket size 约为：

$$
\text{bucket\_size}^\star
= \frac{s}{n_b^\star}
= \sqrt{s w o}
$$
...
- 数据并行：数据批次被分配到多个设备，每个设备计算其各自批次的梯度。这些梯度需要在设备间进行平均。
- 全分片数据并行：优化器状态、梯度和权重在设备间分片。如果我们仅使用DP和FSDP，每个设备都需要从所有其他设备收集权重分片，才能进行前向或反向传播。
- 张量并行：激活值跨一个新的维度进行分片，每个设备计算其自己分片的输出结果。使用张量并行时，我们可以沿着我们正在分片的操作的输入或输出进行分片。如果我们沿着相应的维度对权重和激活进行分片，张量并行可以与FSDP有效结合。
- 流水线并行：模型被按层分割成多个阶段，每个阶段在不同的设备上运行。
- 专家并行性：我们将专家（在混合专家模型中）分配到不同的设备上，并且每个设备计算它们自己专家的输出结果。

在这里我们不讨论 EP。

**问题：communication_accounting**
...
令 \(B\) 表示 **global batch size in tokens**，令：
...
P
=
2\cdot 16384\cdot 53248\cdot 126
=
219{,}848{,}638{,}464
\approx 2.20\times 10^{11}
$$

Adam 训练中需要 FP32 master weights、FP32 gradients，以及 Adam 的两个 FP32 optimizer states。因此一共有 4 份 FP32 状态：
...
16P
\approx
3.518\times 10^{12}\text{ bytes}
\approx
3.52\text{ TB}
$$

如果按 H100 80GB 估算，仅这些 FP32 states 就需要：

$$
\left\lceil
\frac{3.52\text{ TB}}{80\text{ GB}}
\right\rceil
=
44
$$

张 GPU，不包含 activations。

BF16 saved activations 近似为：

$$
A(B)=2BL(d+f)
$$

其中最前面的 \(2\) 来自 BF16 每个元素 2 bytes。代入数值得：

$$
A(B)
=
2B\cdot 126\cdot(16384+53248)
=
...
$$
S=16P\approx 3.52\text{ TB}
$$

BF16 saved activations 为：
...
M_{\text{device}}(B,N_{\text{FSDP}})
=
\frac{S+A(B)/2}{N_{\text{FSDP}}}
+
\frac{A(B)}{2}
$$

如果先忽略 activations，只考虑 FP32 model / gradient / optimizer states，则：
...
如果代入 (c) 中得到的 compute-bound global batch size：
...
A(3925)
=
17{,}547{,}264\cdot 3925
\approx
6.89\times 10^{10}\text{ bytes}
\approx
68.9\text{ GB}
$$
...
$$
M_{\text{device}}<95\text{ GB}
$$

即：

$$
\frac{3552.0}{N_{\text{FSDP}}}
+
34.4
<
95
...
\frac{C}{W_{\text{ici}}}
=
\frac{4.6\times 10^{14}}{1.8\times 10^{11}}
\approx
2555.6
$$

对于 mixed FSDP + tensor parallelism，compute-bound 条件为：

$$
\frac{B}{N}
>
\frac{\alpha^2}{M_XM_Yf}
$$

题目给定：

$$
X=16,\qquad Y=4,\qquad N=XY=64
$$

以及：

$$
M_X=2,\qquad M_Y=1,\qquad f=53248
$$
...
\frac{B}{64}
>
\frac{2555.6^2}{2\cdot 1\cdot 53248}
$$
...
因此每设备 batch size 至少约为：

$$
b=\frac{B}{N}\approx 62\text{ tokens}
$$

对应的 global batch size 为：
...
而 mixed FSDP + tensor parallelism 下，compute-bound 条件可以降低为：
...
原因是 FSDP 主要通信 weights，而 tensor parallelism 主要通信 activations。当 batch size 较小时，activation 通信相对更便宜，因此加入 tensor parallelism 可以降低所需的 per-device batch size。

此外，还可以结合 pipeline parallelism、activation checkpointing、sequence/context parallelism 等方法。Pipeline parallelism 可以把层切分到不同设备上，并通过 microbatching 或 zero-bubble schedules 保持设备利用率；activation checkpointing 可以降低 activation memory；sequence/context parallelism 和 ring attention 可以进一步分摊长上下文训练中的序列维度开销。

整体思路是：用更多 model-parallel 维度替代单纯增大 data-parallel batch size，从而在更小 global batch 下维持高吞吐。

# 3 优化器状态分片

**问题：optimizer_state_sharding、optimizer_state_sharding_accounting**
