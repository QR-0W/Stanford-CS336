...
def main():
    ...
    print(f"最大内存使用: {max_mem / 1e6:.2f} MB")
    ...
    # 保存 vocab 和 merges
    # vocab: {id: bytes} -> {id: hex string}
    vocab_json = {str(k): v.hex() for k, v in vocab.items()}
    with open(OUTPUT_DIR / "tiny_stories_vocab.json", "w", encoding="utf-8") as f:
        json.dump(vocab_json, f, ensure_ascii=False, indent=2)

    # merges: [(bytes, bytes), ...] -> [(hex, hex), ...]
    merges_json = [(p[0].hex(), p[1].hex()) for p in merges]
    with open(OUTPUT_DIR / "tiny_stories_merges.json", "w", encoding="utf-8") as f:
